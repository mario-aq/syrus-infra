...
func handleRequest(ctx context.Context, request events.APIGatewayV2HTTPRequest) (events.APIGatewayV2HTTPResponse, error) {
	...
	if request.RequestContext.HTTP.Method == "GET" {
		...
		// Test connectivity on GET requests
		...
		if resp, err := testClient.Get("https://httpbin.org/get"); err != nil {
			log.Printf("DEBUG: Connectivity test failed on GET: %v", err)
		} else {
			resp.Body.Close()
			log.Printf("DEBUG: Connectivity test succeeded on GET")
		}
...
func sendReceivedMessage(recipientPhoneNumber string) {
	...
	// Create HTTP client with longer timeout and debugging
	client := &http.Client{
	...
	// First test basic connectivity (remove this after debugging)
	testResp, testErr := client.Get("https://httpbin.org/get")
	if testErr != nil {
		log.Printf("DEBUG: Basic connectivity test failed: %v", testErr)
	} else {
		testResp.Body.Close()
		log.Printf("DEBUG: Basic connectivity test succeeded")
	}
